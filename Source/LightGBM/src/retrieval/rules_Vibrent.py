...
class OrderHistory(PersonalRetrieveRule):
    ...
    def retrieve(self) -> pd.DataFrame:
        ...
        # Get max date per customer
        max_date = df.groupby("customer_id")["t_dat"].transform("max")
        df["diff_days"] = (max_date - df["t_dat"]).dt.days

        # Keep records within recent N days
        df = df[df["diff_days"] < self.days]

        # Sort to keep most recent purchases
        df = df.sort_values(["customer_id", "t_dat"], ascending=[True, False])

        # Assign score (higher = more recent)
        df["score"] = -df["diff_days"]
        df["method"] = f"OrderHistory_{self.name}"

        if self.n is not None:
            # Get top N recent items per customer
            df = df.groupby("customer_id").head(self.n)

        return df[["customer_id", self.iid, "score", "method"]].drop_duplicates()
...
class UserGroupTimeHistory(UserGroupRetrieveRule):
    def __init__(
        ...
        cols = ["customer_id", self.iid] + cat_cols
        if time_col:
            cols.append(time_col)
        self.trans_df = trans_df[cols]

    def retrieve(self):
        df = self.trans_df.copy()

        if self.time_col and self.days:
            df[self.time_col] = pd.to_datetime(df[self.time_col])
            max_date = df[self.time_col].max()
            df = df[df[self.time_col] >= max_date - pd.Timedelta(days=self.days)]
        ...
        df["rank"] = df.groupby(self.cat_cols).cumcount() + 1
        if self.scale:
...
class UserGroupSaleTrend(UserGroupRetrieveRule):
    ...
    def retrieve(self):
        ...
        past = df[df["dat_gap"] > self.days - 1].copy()
        recent = df[df["dat_gap"] <= self.days - 1].copy()

        past["count"] = 1
        recent["count"] = 1

        past_group = past.groupby([*self.cat_cols, self.iid])["count"].sum().reset_index()
        recent_group = recent.groupby([*self.cat_cols, self.iid])["count"].sum().reset_index()

        trend_df = pd.merge(recent_group, past_group, on=[*self.cat_cols, self.iid], how="left")
        trend_df["count_y"].fillna(0, inplace=True)
        trend_df["trend"] = (trend_df["count_x"] - trend_df["count_y"]) / (trend_df["count_y"] + 1)

        trend_df = trend_df[trend_df["trend"] > self.t]
        trend_df = trend_df.sort_values(by=["count_x", "trend"], ascending=False).reset_index(drop=True)
        trend_df["rank"] = trend_df.groupby(self.cat_cols).cumcount() + 1
        trend_df = trend_df[trend_df["rank"] <= self.n]

        trend_df["method"] = f"UGSaleTrend_{self.name}"
        trend_df["score"] = trend_df["trend"]

        result = trend_df[[*self.cat_cols, self.iid, "method", "score"]]
...
class SaleTrendTopN(GlobalRetrieveRule):
    """Trả về top-N sản phẩm trending dựa vào xu hướng bán hàng."""
    ...
    def retrieve(self):
        ...
        # Giai đoạn gần và xa
        recent_period = item_sale[item_sale["dat_gap"] <= self.days - 1].copy()
        past_period = item_sale[
            (item_sale["dat_gap"] > self.days - 1) &
            (item_sale["dat_gap"] <= 2 * self.days - 1)
        ].copy()

        recent_period["count"] = 1
        past_period["count"] = 1
        recent_sales = recent_period.groupby(self.iid)["count"].sum().reset_index()
        past_sales = past_period.groupby(self.iid)["count"].sum().reset_index()

        df = pd.merge(recent_sales, past_sales, on=self.iid, how="left", suffixes=("_recent", "_past"))
        df["count_past"] = df["count_past"].fillna(0)
        df["trend"] = (df["count_recent"] - df["count_past"]) / (df["count_past"] + 1e-6)

        # Lấy top-N sản phẩm
        df = df.sort_values(by=["trend", "count_recent"], ascending=False).reset_index(drop=True).iloc[:self.n]

        return df[[self.iid, "count_recent", "count_past", "trend"]]
...
class OutOfStock(FilterRule):
    ...
    def __init__(self, trans_df: pd.DataFrame, item_id: str = "article_id"):
        self.iid = item_id
        mask = trans_df["t_dat"] >= "2023-10-01"
    ...
    def _off_stock_items(self) -> List[int]:
        ...
        sale = sale.fillna(0)
        mask = ((sale["2023_10"] - sale["2023_10"]) / sale["2023_10"]) < -0.8
        mask2 = sale["2023_10"] == 0
