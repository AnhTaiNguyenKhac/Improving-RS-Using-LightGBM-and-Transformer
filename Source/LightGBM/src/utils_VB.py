...
def calc_valid_date(week_num: int, last_date: str = "2023-10-30") -> Tuple[str]:
    ...
    last_date : str, optional
        The last day, by default ``"2023-10-22"``.
...
#     return candidates
def reduce_mem_usage(df: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
    """Reduce memory usage by changing feature dtype with safer type conversion."""
    start_mem_usg = df.memory_usage().sum() / 1024**2
    if verbose:
        print("Memory usage of dataframe is :", start_mem_usg, " MB")
    ...
    for col in df.columns:
        if df[col].dtype == object:  # Skip strings
            continue
        ...
        if col == 'rank':
            ...
            if verbose:
                print("Special handling for rank column")
                print("dtype after: ", df[col].dtype)
                print("******************************")
            continue

        # Check if column can be converted to integer
        is_int = False
        if pd.api.types.is_integer_dtype(col_type):
            is_int = True
        else:
            asint = df[col].fillna(0).astype(np.int64)
            result = (df[col] - asint).abs().sum()
            if result < 0.01:  # All values are effectively integers
                is_int = True
        ...
        if is_int:
            ...
            # Float conversion - only downcast if safe
            if mn > np.finfo(np.float16).min and mx < np.finfo(np.float16).max:
                ...
                # Keep as float64 if values are too large/small
                pass
...
def merge_week_data(
    ...
    # Handle labels for non-test data
    if week_num != 0:
        start_date, end_date = calc_valid_date(week_num)
        mask = (start_date <= data["inter"]["t_dat"]) & (
            data["inter"]["t_dat"] < end_date
        ...
        candidates["label"] = candidates["label"].fillna(0).astype(np.uint8)
    ...
    # Convert user features with explicit handling
    for feat in user_feats:
        if feat in ['age', 'FN', 'Active']:
            candidates[feat] = candidates[feat].fillna(-1).astype(np.int8)
        else:
            candidates[feat] = candidates[feat].astype('category').cat.codes.astype(np.int8)
    ...
    # Final memory reduction with safe conversion
    candidates, _ = reduce_mem_usage(candidates)
...
def calc_embd_similarity(
    ...
    for batch in tqdm(range(0, candidate.shape[0], batch_size)):
        # Gán trước, không phụ thuộc vào `sub`
        tmp_users = candidate.loc[batch : batch + batch_size - 1, "customer_id"].values
        tmp_items = candidate.loc[batch : batch + batch_size - 1, item_id].values

        if sub:
            tmp_users = np.clip(tmp_users - 1, 0, user_embd.shape[0] - 1)
            tmp_items = np.clip(tmp_items - 1, 0, item_embd.shape[0] - 1)

        tmp_user_embd = np.expand_dims(user_embd[tmp_users], 1)  # (batch, 1, dim)
        tmp_item_embd = np.expand_dims(item_embd[tmp_items], 2)  # (batch, dim, 1)
