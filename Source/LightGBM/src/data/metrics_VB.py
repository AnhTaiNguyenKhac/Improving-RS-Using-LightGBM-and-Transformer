...
# Metrics: Handle NaN/float in predictions
...
def _rk(actual, predicted, k=10):
    ...
    return sum(1 for r in actual if r in predicted) / len(actual) if len(actual) > 0 else 0.0
...
def hr_at_k(actual: Iterable, predicted: Iterable, k: int = 10) -> float:
    ...
    for i, actual_i in enumerate(actual):
        ...
        if predicted_i is None or isinstance(predicted_i, float) or not predicted_i:
            continue
        if isinstance(predicted_i, str):
            predicted_i = predicted_i.split()
        for p in predicted_i[:k]:
            if p in actual_i:
                count += 1
                break
    return count / len(actual) if len(actual) > 0 else 0.0
